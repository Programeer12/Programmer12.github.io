        ...
        try {
            ...
            if (empty($error_message)) {
                ...
                if (!empty($new_password)) {
                    ...
                    } elseif (!password_verify($current_password, $teacher_profile['password'])) {
                ...
                if (empty($error_message)) {
                    ...
                    if ($update_password) {
                        ...
                        $stmt->execute([$name, $email, $username, $hashed_password, $teacher_id]);
                    } else {
                        $stmt = $pdo->prepare("UPDATE users SET name = ?, email = ?, username = ?, updated_at = NOW() WHERE id = ?");
                        $stmt->execute([$name, $email, $username, $teacher_id]);
                    }
...
try {
    // Get join date
    $join_date = new DateTime($teacher_profile['created_at']);
    ...
    // Count total students who have submitted assignments to this teacher
    $stmt = $pdo->prepare("SELECT COUNT(DISTINCT s.student_id) FROM submissions s JOIN assignments a ON s.assignment_id = a.id WHERE a.teacher_id = ?");
    $stmt->execute([$teacher_id]);
    $total_students = $stmt->fetchColumn();
    
    // Count total submissions to this teacher's assignments
    $stmt = $pdo->prepare("SELECT COUNT(s.id) FROM submissions s JOIN assignments a ON s.assignment_id = a.id WHERE a.teacher_id = ?");
    $stmt->execute([$teacher_id]);
    $total_submissions = $stmt->fetchColumn();
    
    // Count pending submissions (not graded yet)
    $stmt = $pdo->prepare("SELECT COUNT(s.id) FROM submissions s JOIN assignments a ON s.assignment_id = a.id WHERE a.teacher_id = ? AND s.status = 'submitted'");
    $stmt->execute([$teacher_id]);
...
<head>
    <meta charset="UTF-8">
    <title>My Profile - EduAssign Teacher</title>
    ...
    <header>
        <div class="header-container">
            ...
            <nav>
                ...
                <a href="teacher_profile.php" class="active"><i class="fas fa-user"></i> Profile</a>
        ...
        <div class="profile-container">
            ...
            <div class="profile-sidebar">
                <div class="profile-avatar">
                    ...
                    <div class="profile-name"><?php echo htmlspecialchars($teacher_profile['name']); ?></div>
                    <div class="profile-role">Teacher</div>
                </div>
                
                <div class="info-card">
                    <h4><i class="fas fa-info-circle"></i> Account Information</h4>
                    <p>Teacher ID: <?php echo htmlspecialchars($teacher_profile['id']); ?></p>
                    <p>Username: <?php echo htmlspecialchars($teacher_profile['username']); ?></p>
                    <p>Email: <?php echo htmlspecialchars($teacher_profile['email']); ?></p>
                    <p>Subject: <?php echo htmlspecialchars($teacher_profile['subject'] ?: 'Not Set'); ?></p>
                    <p>Course: <?php echo htmlspecialchars($teacher_profile['course'] ?: 'Not Set'); ?></p>
                    <p>Joined: <?php echo $join_date->format('M j, Y'); ?></p>
                    <p>Status: <?php echo ucfirst($teacher_profile['status']); ?></p>
                    <p>Last Updated: <?php echo (new DateTime($teacher_profile['updated_at']))->format('M j, Y g:i A'); ?></p>
                </div>
                ...
                <div class="profile-stats">
                    ...
                    <div class="stat-item">
                        <span class="stat-label"><i class="fas fa-users"></i> Students Taught</span>
                        <span class="stat-value"><?php echo $total_students; ?></span>
                    </div>
                    <div class="stat-item">
                        <span class="stat-label"><i class="fas fa-file-alt"></i> Total Submissions</span>
                        <span class="stat-value"><?php echo $total_submissions; ?></span>
                    </div>
                    <div class="stat-item">
                        <span class="stat-label"><i class="fas fa-clock"></i> Pending Grading</span>
                        <span class="stat-value"><?php echo $pending_grading; ?></span>
                    </div>
                    <div class="stat-item">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" id="name" name="name" class="form-control" 
                                   value="<?php echo htmlspecialchars($teacher_profile['name']); ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="email"><i class="fas fa-envelope"></i> Email Address</label>
                            <input type="email" id="email" name="email" class="form-control" 
                                   value="<?php echo htmlspecialchars($teacher_profile['email']); ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="username"><i class="fas fa-at"></i> Username</label>
                            <input type="text" id="username" name="username" class="form-control" 
                                   value="<?php echo htmlspecialchars($teacher_profile['username']); ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="status"><i class="fas fa-check-circle"></i> Account Status</label>
                            <input type="text" id="status" class="form-control" 
                                   value="<?php echo ucfirst($teacher_profile['status']); ?>" disabled>
                        </div>
                        <div class="form-group">
                            <label for="subject"><i class="fas fa-book"></i> Subject</label>
                            <input type="text" id="subject" class="form-control" 
                                   value="<?php echo htmlspecialchars($teacher_profile['subject'] ?: ''); ?>" disabled>
                        </div>
                        <div class="form-group">
                            <label for="course"><i class="fas fa-graduation-cap"></i> Course</label>
                            <input type="text" id="course" class="form-control" 
                                   value="<?php echo htmlspecialchars($teacher_profile['course'] ?: ''); ?>" disabled>
                        </div>
                    </div>
